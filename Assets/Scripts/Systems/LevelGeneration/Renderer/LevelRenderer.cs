...
namespace DungeonDraws.Scripts.Systems.LevelGeneration.Renderer
{
    ...
    {
        private LevelGeneratorBehaviour _behavior;
        private LevelRenderer(LevelGeneratorBehaviour behaviour) {
            _behavior = behaviour;
        }
            ...
            for (int row = 0; row < map.GetLength(0); row++) {
                for (int col = 0; col < map.GetLength(1); col++) {
                    ...
                    if (type == DetailedTileType.Floor) {
                        ...
                        GameObject prefab = _behavior._cornerInPrefab;
                        float xPos = col * floorSpan;
                        float zPos = -row * floorSpan;
                        float yRot = 180f;
                        _instantiate(prefab, xPos, zPos, yRot, boardHolder, row, col, type.ToString());
        ...
        public static LevelRenderer newInstance(LevelGeneratorBehaviour behaviour) {
            LevelRenderer renderer = ScriptableObject.CreateInstance<LevelRenderer>();
            renderer.setBehaviour(behaviour);
            return renderer;
        }

        private void setBehaviour(LevelGeneratorBehaviour dungeonGeneratorBehaviour) {
