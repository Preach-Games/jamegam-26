...
namespace DungeonDraws.Scripts.Systems.LevelGeneration.Domain
{
    public class Board
    {
        ...
        {
            if (!aSquare.isWithin(_grid))
                return false;
            //Avoid checking collision with the last one as not necessary
            for (int i = 0; i < _roomsAndCorridors.Count - 1; i++)
            {
                IShape each = _roomsAndCorridors[i];
                if (aSquare.collidesWith(each))
                    return false;
            }
            return true;
        }
        ...
        public int[,] asTilesMatrix(IBoardPlotter plotter)
        {
        ...
        public Board resize(int mapMargin)
        {
            ...
            foreach (IShape each in _roomsAndCorridors)
            {
                ...
                else
                {
                    ...
                    Corridor.Orientation orient = c.isOrizontal()
        ...
        public Board crop(int marginToAddAfterCrop)
        {
            ...
            int rows =
                downerBottomRightVert.row() - upperTopLeftVert.row() + 1 + marginToAddAfterCrop * 2;
            int cols =
                righterBottomRightVert.col()
                - lefterTopLeftVert.col()
                + 1
                + marginToAddAfterCrop * 2;
